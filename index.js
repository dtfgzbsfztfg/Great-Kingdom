...
const { buildBoardMessage, scoreLines } = require('./src/embeds');
...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    if (commandName === 'king-start') {
      ...
      const message = await buildBoardMessage(
        result.entry,
        `대국이 시작되었습니다! <@${user.id}>(파랑) vs <@${opponent.id}>(주황)`,
      );
    ...
    if (commandName === 'king-board') {
      const message = await buildBoardMessage(entry);
      await interaction.reply(message);
      return;
    }
    ...
    if (commandName === 'king-move') {
      ...
      const message = await buildBoardMessage(entry, extra);
      await interaction.reply(message);
    ...
    if (commandName === 'king-resign') {
      ...
      const message = await buildBoardMessage(entry, `<@${user.id}> (${colorName(myColor)})가 기권했습니다.`);
      await interaction.reply(message);
      gameManager.end(channelId);
      return;
    }
